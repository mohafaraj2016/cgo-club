...
const TXT = {
  ...
  it: {
    ...
    selectedN: "Selezionati",
    addManual: "+ Aggiungi giocatore (manuale)",
    name: "Nome",
    pos: "Ruolo",
    power: "Forza",
    vittorie: "Vittorie (%)",
...
function wilsonLowerBound(wins, n, z = 1.96){ if(n<=0) return 0; const p=Math.max(0,Math.min(1,wins/n)); const z2=z*z; const d=1+z2/n; const c=p+z2/(2*n); const m=z*Math.sqrt((p*(1-p)+z2/(4*n))/n); return clamp01((c-m)/d); }

/* — same logic as FIFA page to compute Power (rating) — */
function ratingLikeCard({ pos, presenze, winPct01, astPM, golPM }) {
  ...
  return { rating, winCons01 };
}

const FORMATIONS_BY_SIZE = {
  5:["1-3-1","2-2-1","1-2-2"],
  6:["1-3-2","2-2-2","1-2-3"],
  7:["1-2-3-1","2-3-1-1","1-3-2-1"],
  8:["1-3-3-1","1-2-3-2","2-3-2-1"],
...
export default function Selection({ lang = "en" }){
  ...
  const [mVitt, setMVitt] = useState("");

  // single exportRef (the previous error was from declaring it twice)
  ...
  useEffect(()=>{ (async()=>{
    try{
      ...
      const players = body.map((r,i)=>{
        ...
        const { rating, winCons01 } = ratingLikeCard({ pos, presenze:pres, winPct01:win, astPM:ast, golPM:gol });
        return {
          id:`cgo-${i}-${surname}-${pos}`,
  ...
  const selectedCount = poolPlaying.length;
  ...
  // manual add into main list
  const [manualSeq, setManualSeq] = useState(0);
  function addManualPlayer(){
    const name = str(mName);
    const pos  = str(mPos).toUpperCase();
    const power = Math.max(30, Math.min(99, parseInt(mPower,10) || 60));
    const vitt = Math.max(0, Math.min(100, parseFloat(mVitt) || 0));
    if (!name) return;
    ...
    setRows(r => [...r, p]);
    setPlaying(s => ({ ...s, [id]: true }));
    setAssign(s => ({ ...s, [id]: "" }));
    setMName(""); setMPos(""); setMPower(""); setMVitt("");
  }

  const teamA = useMemo(()=>{
    const base = poolPlaying.filter(p=>assign[p.id]==='A');
    return [...base, ...extA.map((p,i)=>({ id:`extA-${i}`, name:p.name||"EXT", pos:(p.pos||"").toUpperCase(), power:+p.power||60, winPct:(+p.vitt||0)/100 }))];
  },[poolPlaying,assign,extA]);
  const teamB = useMemo(()=>{
    const base = poolPlaying.filter(p=>assign[p.id]==='B');
    return [...base, ...extB.map((p,i)=>({ id:`extB-${i}`, name:p.name||"EXT", pos:(p.pos||"").toUpperCase(), power:+p.power||60, winPct:(+p.vitt||0)/100 }))];
  ...
  const metrics = useMemo(()=>{
    ...
    const tot = sA+sB || 1;
    const pctA = Math.round(10000*(sA/tot))/100;
    const pctB = Math.round(10000*(sB/tot))/100;

    const meanA = teamA.reduce((s,p)=>s+(p.winPct ?? 0),0) / (teamA.length || 1);
    const meanB = teamB.reduce((s,p)=>s+(p.winPct ?? 0),0) / (teamB.length || 1);

    return { sA, sB, pctA, pctB, wPctA: Math.round(meanA*10000)/100, wPctB: Math.round(meanB*10000)/100 };
  },[teamA,teamB]);
  ...
  async function doExportPNG(){
    ...
    const a = document.createElement("a"); a.href = dataUrl; a.download = `cgo_selection_${perSide}v${perSide}.png`; a.click();
  }
  ...
  return (
    <section className="home-layout">
      <div className="card" style={{ display:"grid", gap:16 }}>
        ...
        {/* Step 2: Availability + counter + manual add */}
        <div className="card" style={{ display:"grid", gap:12 }}>
          <div className="h2">
            {t(lang,"step2")} <span className="pill">{t(lang,"selectedN")}: {selectedCount} / {perSide}</span>
          </div>

          {/* Manual add row */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr 90px 110px 130px 90px", gap:8, alignItems:"center" }}>
            <input className="btn" placeholder={t(lang,"name")} value={mName} onChange={e=>setMName(e.target.value)} />
            <input className="btn" placeholder={t(lang,"pos")} value={mPos} onChange={e=>setMPos(e.target.value)} />
            <input className="btn" placeholder={t(lang,"power")} value={mPower} onChange={e=>setMPower(e.target.value)} />
            <input className="btn" placeholder={t(lang,"vittorie")} value={mVitt} onChange={e=>setMVitt(e.target.value)} />
            <button className="btn" onClick={addManualPlayer}>{t(lang,"add")}</button>
          </div>
          ...
          {status!=="loading" && (
            <div style={{ display:"grid", gap:6 }}>
              {rows.map(p=>(
                <label key={p.id} style={{ display:"grid", gridTemplateColumns:"26px 1fr 160px 110px", gap:8, alignItems:"center" }}>
                  <input type="checkbox" checked={!!playing[p.id]} onChange={()=>togglePlaying(p.id)} />
                  <span className="p">
                    <b>{p.name}</b> {p.pos?`(${p.pos})`:""} · Pwr {p.power}
                  </span>
                  <span className="pill soft">{t(lang,"vittorie")}: {Math.round((p.winPct||0)*100)}%</span>
        ...
        {/* Step 3: Captains + Team Names */}
        <div className="card" style={{ display:"grid", gap:12 }}>
          <div className="h2">{t(lang,"step3")}</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div>
              <div className="p" style={{ fontWeight:700, color:"var(--teamA)" }}>{t(lang,"teamA")}</div>
              <input className="btn" value={teamNameA} onChange={(e)=>setTeamNameA(e.target.value)} placeholder={t(lang,"teamName")} />
              <select className="btn" value={captA} onChange={e=>setCaptainA(e.target.value)} style={{ marginLeft:8 }}>
                <option value="">—</option>
            ...
            <div>
              ...
              <input className="btn" value={teamNameB} onChange={(e)=>setTeamNameB(e.target.value)} placeholder={t(lang,"teamName")} />
              <select className="btn" value={captB} onChange={e=>setCaptainB(e.target.value)} style={{ marginLeft:8 }}>
                <option value="">—</option>
        ...
        {/* Step 4: Manual assignment with Vittorie + Power bar */}
        <div className="card" style={{ display:"grid", gap:10 }}>
          <div className="h2">{t(lang,"step4")}</div>
          <div style={{ display:"grid", gap:8 }}>
            {poolPlaying.map(p=>(
              <div key={p.id} style={{ display:"grid", gridTemplateColumns:"minmax(200px,1fr) 120px 110px 130px", gap:8, alignItems:"center" }}>
                <div className="p"><b>{p.name}</b> {p.pos?`(${p.pos})`:""}</div>
                <div className="pill soft">{t(lang,"vittorie")}: {Math.round((p.winPct||0)*100)}%</div>
                <div className="bar">
                  <div className="bar-inner green" style={{ width: `${p.power||0}%` }}/>
                </div>
        ...
        <div className="card" style={{ display:"grid", gap:12 }}>
          ...
          <div ref={exportRef} style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            <div>
              <div className="p" style={{ fontWeight:700, color:"var(--teamA)" }}>{teamNameA}</div>
              <select className="btn" value={formationA} onChange={e=>setFormationA(e.target.value)}>
                {(formations).map(f=> <option key={f} value={f}>{f}</option>)}
              </select>
              <PitchHalf title={`${teamNameA}`} formation={parseFormation(formationA)} players={teamA.slice(0,perSide)} color="var(--teamA)" />
            </div>
            <div>
              <div className="p" style={{ fontWeight:700, color:"var(--teamB)" }}>{teamNameB}</div>
              <select className="btn" value={formationB} onChange={e=>setFormationB(e.target.value)}>
                {(formations).map(f=> <option key={f} value={f}>{f}</option>)}
              </select>
              <PitchHalf title={`${teamNameB}`} formation={parseFormation(formationB)} players={teamB.slice(0,perSide)} color="var(--teamB)" />
            </div>
          </div>
        </div>
